...
//				var freq = (rel_x * (total_freq) + 0) >> 0; // + 16 (min freq)
	...
	var ParagraphicModal = function ( app, custom_presets ) {
		...
		var x = new PKAudioFXModal ({
			...
			presets:[
					{name:'旧电话',val:'1,highpass,0,5800,5.8,1,lowpass,0,7060,5'}
			],
		...
		var TempoMetro = function ( app, modal ) {
			...
			function _make_ui ( q ) {
				...
				el_drawer.innerHTML = '<div class="pk_row">'+
					...
					'<label for="xxcjgs">加强节拍器点击</label></div>' + 

					'<div class="pk_row">'+
					'<a class="pk_modal_a_bottom" style="display:inline-block;float:none">节拍器</a>'+
					'<a class="pk_modal_a_bottom" style="display:inline-block;float:none">播放轨道</a>'+
					'<a class="pk_modal_a_bottom" style="display:inline-block;float:none">播放两者</a>'+
					'</div>';
		...
		var TempoTap = function ( app, modal ) {
			...
			function _make_ui ( q ) {
				...
				el_drawer.innerHTML = '<div class="pk_row pk_pgeq_els">' + 
					...
					'<input style="margin-left:2px;min-width:64px;max-width:64px" '+
					'type="text" class="pk_val pk_gain" value="-">'+
					'<a class="pk_modal_a_bottom" style="display:inline-block;float:none">重置</a>'+
					'<a class="pk_modal_a_bottom" style="display:inline-block;float:none">播放音轨</a>'+
					'<a class="pk_modal_a_bottom" style="display:inline-block;float:none">循环</a>'+
					'</div>'+

					'<div><div id="pk_tmp_tap">'+
					'<span style="opacity:0" class="pk_obj2">已清除...</span>'+
...
//						ctx.drawImage (tempCanvas, 0, 0, width, 100, -(raw_pixels.toFixed(1)/1), 0, width, 100);
		...
		var x = new PKAudioFXModal ({
			...
			body: '<div class="pk_tbs">' +
				'<a class="pk_tbsa pk_inact">节奏估计</a>' +
				'<a class="pk_tbsa">节奏点击</a>' +
				'<a class="pk_tbsa">节拍器</a></div>',
		...
		var x = new PKAudioFXModal ({
			...
			body: '<div class="pk_rec" style="user-select:none">' +
				...
				'<div class="pk_row">' +
				  ...
				  '<div style="float:left;margin-left:20px;"><label>时间</label>' +
				...
				'</div>' +
				'<div class="pk_row">' +
				  '<a class="pk_tbsa pk_inact" style="text-align: center;">开始录音</a>' +
				  '<a class="pk_tbsa pk_inact" style="margin-left: 24px; text-align: center;">暂停</a>' +
				'</div>' +
				'<div class="pk_row">' +
					'<a class="pk_tbsa" style="float:left;display:none;text-align:center;box-shadow:0 0 7px #3a6b79 inset;">打开录音</a>' +
					'<a class="pk_tbsa" style="float:left;display:none;margin-left: 24px; text-align: center;">附加到现有</a>' +
				'</div>' +
				'</div>',
...
//			}],
	...
	//						freqctx.drawImage (tempCanvas, 0, 0, width, 100, -(raw_pixels.toFixed(1)/1), 0, width, 100);
					...
					btn_pause.onclick = function () {
						...
						btn_pause.innerText = is_paused ? '结束暂停' : '暂停';
					};
