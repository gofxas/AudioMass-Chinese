	...
	function PKUI_FX ( app ) {
		...
		app.listenFor ('RequestFXUI_Gain', function () {
			...
			preview: function ( q ) {
			  ...
			  body:'<div class="pk_row" style="border:none"><label>百分比</label>' + 
		...
		app.listenFor ('RequestActionFXUI_Speed', function () {
			...
			preview: function ( q ) {
			  ...
			  body:'<div class="pk_row" style="border:none"><label>播放速率</label>' + 
		...
		app.listenFor ('RequestFXUI_Silence', function () {
			var x = new PKSimpleModal({
			  ...
			  body:'<div class="pk_row"><input type="radio" class="pk_check" id="ifeq" name="rdslnc" value="beginning">'+ 
				...
				'<div class="pk_row"><label>静音（秒）</label>'+
		...
		app.listenFor ('RequestActionFXUI_Compressor', function () {
			...
			ondestroy: function ( q ) {
				...
				presets:[
					...
					{name:'娱乐性的',val:'-6,2,2.5,0.002,0.05'},
					{name:'虚线失真',val:'-45,26,2.05,0.233,0.0'},
					{name:'混沌失真',val:'-60,14,11.07,0.036,0.00'}
				],
				custom_pres:custom_presets.Get (filter_id),
			preview: function ( q ) {
				var inputs = q.el_body.getElementsByTagName('input');
		...
		app.listenFor ('RequestActionFXUI_Normalize', function () {
			...
			var x = new PKSimpleModal({
			  ...
			  body:'<div class="pk_row">'+
				...
				'<label for="vhcjgs">平均 L/R </label></div>' + 
				'<div class="pk_row" style="border:none"><label>调整到</label>'+
		...
		app.listenFor ('RequestActionFXUI_Delay', function () {
			...
			preview: function ( q ) {
			  ...
			  body:'<div class="pk_row"><label class="pk_line">延迟时间</label>' + 
		...
		app.listenFor ('RequestActionFXUI_Distortion', function () {
			...
			preview: function ( q ) {
			  ...
			  body:'<div class="pk_row"><label class="pk_line">增值</label>' + 
		...
		app.listenFor ('RequestActionID3', function (flag, new_tags) {
				...
				new PKSimpleModal({
				  ...
				  body:'<input type="file" accept="audio/*" />'+
				  	'<div class="pk_row pk_ttx">选择要查看音频元数据的文件！</div>',
				  setup:function( q ) {
				  		var input  = q.el_body.getElementsByTagName ('input')[0];
		...
		app.listenFor ('RequestSavePreset', function () {
			...
			if (preset_obj.val.length > 0)
			{
					...
					var title = '保存过滤器的自定义预设 "' + curr_filter_ui.id + '"';
					if (!is_new) {
						var cname = custom_obj.name;
						title = '编辑自定义预设 "' + cname + '", 为过滤器 "' + curr_filter_ui.id + '"';
					}
					...
					new PKSimpleModal({
					  ...
					  body:'<label for="k07">预设名称</label>' + 
						'<input style="min-width:340px" maxlength="16" placeholder="请输入名称, 例如: My Preset" ' + default_txt + ' class="pk_txt" type="text" id="k07" />',
					  setup:function( q ) {
					  	  	// app.fireEvent ('RequestPause');
		...
		app.listenFor ('RequestShowFreqAn', function ( url, args_arr ) {
			...
			if (!type)
			{
				var makePopup = function ( dat ) {
					...
					if (!wnd) {
						OneUp ('请允许AudioMass弹出窗口!', 3600, 'pk_r');
						return ;
					}
