(function ( w, d, PKAE ) {
				...
				children : [
					{
						...
						action: function () {
								new PKSimpleModal({
								  ...
								  body:'<div class="pk_row"><label for="k0">文件名称</label>' + 
									...
									'<label class="pk_lblmp3" for="k5">仅导出选择</label></div>',
								  ...
								  buttons:[
									{
										...
										callback: function( q ) {
											...
											{
												OneUp ('输入了无效的URL', 1100);
											}
											// -
										}
									}
								  ],
								  body:'<label for="k00">插入URL</label>' + 
									'<input style="min-width:250px" placeholder="请插入URL" class="pk_txt" type="text" id="k00" />',
								  setup:function( q ) {
						...
						action: function ( e ) {
							...
							new PKSimpleModal ({
								...
								body:'<p>请选择来源...</p>' +
									...
									'<label style="vertical-align:top" class="pk_lblsel" for="sl2">选中的'+
						...
						setup: function ( obj ) {
							...
							app.listenFor ('DidStoreDB', function ( obj, e ) {
									...
									var txt = '<div style="padding:2px 0">id: ' + name + '</div>'+
										...
										'&nbsp;&nbsp;&nbsp;'+
										'<span>chan: ' + (obj.chans === 1 ? '单声道' : '立体声') + '</span></div>'+
...

					{
						...
						action: function ( e ) {
							...
							if (!app.fls.on) app.fls.Init (function(err){if(err){alert("数据错误")}});
							else app.fireEvent ('DidOpenDB', app.fls);
						},
						setup: function () {
							var source = {};
			...
			},
			{
				...
				children:[
...

					{
						name:'频率分析仪',
						action: function ( obj ) {
							app.fireEvent ('RequestShowFreqAn', 'eq', [1,1]);
						},
						setup: function ( obj ) {
...

					{
						name:'频谱分析仪',
						action: function ( obj ) {
							app.fireEvent ('RequestShowFreqAn', 'sp', [1,1]);
						},
						setup: function ( obj ) {
							app.listenFor ('DidToggleFreqAn', function ( url, val ) {
								if (url !== 'sp') return ;
					...
					{
						name:'中心到光标 <span class="pk_shrtct">[Tab]</span>',
						action: function ( obj ) {
				...
				children:[
					{
						...
						action : function () {
							...
							var message = '这将打开一个新窗口，该窗口将尝试将本地版本存储在您的浏览器中'; // nicer text
	...
	function _makeUIMainView ( UI, app ) {
		...
		ttmp.innerHTML = '将音频拖放在此窗口中,还可以<a style="white-space:nowrap;font-size:0.94em;" '+
		'onclick="PKAudioEditor.fireEvent('+"'RequestLoadLocalFile'"+')">打开文件</a>或' +'<a style="white-space:nowrap;font-size:0.94em;" '+
		'onclick="PKAudioEditor.engine.LoadSample()">导入示例</a>';
	...
	function _makeUIToolbar (UI) {
		...
		btn_stop.innerHTML = '<span>停止 (Space)</span>';
		...
		btn_play.innerHTML = '<span>播放 (Space)</span>';
		...
		btn_pause.innerHTML = '<span>暂停 (Shift+Space)</span>';
		...
		btn_loop.innerHTML = '<span>循环（L）</span>';
		...
		btn_back_jump.innerHTML = '<span>快退（←）</span>';
		...
		btn_front_jump.innerHTML = '<span>快进（→）</span>';
		...
		btn_back_total.innerHTML = '<span>退到开始（Shift + ←）</span>';
		...
		btn_front_total.innerHTML = '<span>快进结束（Shift + →）</span>';
		...
		btn_rec.innerHTML = '<span>录制（Shift + C）</span>';
		...
		setTimeout(function () {
			...
			main_context.addOption ('设置音量/增益', function( e ) {
			...
			main_context.addOption ('插入静音', function( e ) {
		...
		copy_btn.innerHTML = '<span>复制选择（Shift + C）</span>';
		...
		paste_btn.innerHTML = '<span>粘贴选择（Shift + V）</span>';
		...
		cut_btn.innerHTML = '<span>剪切选择 (Shift + X)</span>';
		...
		silence_btn.innerHTML = '<span>插入静音 (Shift + N)</span>';
		...
		btn_clear_selection.innerHTML = '<span>清空选择 (快捷键：Q)</span>';
